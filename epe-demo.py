...
def RenderConfigFiles():
	...
	script_dir = os.path.dirname(__file__)
	rel_path = "exabgp-ingress-receiving-peer.conf"
	abs_file_path = os.path.join(script_dir, rel_path)
	...
	topo_vars = yaml.load(file.read())
	file.close()
	template_open = open("exabgp-egress-advertising-peer-conf.j2")
	ingress_template = Template(template_open.read())
	template_output = ingress_template.render(topo_vars)
	script_dir = os.path.dirname(__file__)
	rel_path = "exabgp-egress-advertising-peer.conf"
	abs_file_path = os.path.join(script_dir, rel_path)
	with open(abs_file_path, "wb") as f:
		f.write(template_output)
	f.close()
...
def bgpandimportantapps():
	...
	os.system("exabgp exabgp-egress-advertising-peer.conf exabgp-ingress-receiving-peer.conf")
